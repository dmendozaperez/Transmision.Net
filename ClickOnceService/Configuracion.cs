...
namespace ClickOnceService
{
    public partial class Configuracion : Form
    {
        ...
        private void cargar_xstore()
        {
            ...
            try
            {
                if (File.Exists(@_ruta_produccion))
                {
                    ...
                    lblproduccion_X.Text = infofile_pro.Name + " " + version_pro;
                }
                FileInfo infofile_des = new FileInfo(@_ruta_desarrollo);
                var fvi_des = FileVersionInfo.GetVersionInfo(@_ruta_desarrollo);
                var version_des = fvi_des.FileVersion;

                lbldesarrollo_X.Text = infofile_des.Name + " " + version_des;
            }
            catch (Exception exc)
            {
        ...
        private void btnrefresh_X_Click(object sender, EventArgs e)
        {
            cargar_xstore();
        }

        private void btnactualizar_X_Click(object sender, EventArgs e)
        {
            _actualizar_X();
        }
        private void _actualizar_X()
        {
            ...
            {
                string version_pro = "";
                if (File.Exists(@_ruta_produccion + "\\" + "Transmision.Net.Basico.Oracle.dll"))
                {
                    var fvi_pro = FileVersionInfo.GetVersionInfo(@_ruta_produccion + "\\" + "Transmision.Net.Basico.Oracle.dll");
                    version_pro = fvi_pro.FileVersion;
                }
                var fvi_des = FileVersionInfo.GetVersionInfo(@_ruta_desarrollo + "\\" + "Transmision.Net.Basico.Oracle.dll");
                var version_des = fvi_des.FileVersion;

                if (version_des != version_pro)
                {
                    DialogResult resulado = MessageBox.Show("Esta seguro Actualizar la version de la dll del servicio a Produccion XSTORE", "Bata Peru Sistema...", MessageBoxButtons.OKCancel, MessageBoxIcon.Information);
                    if (resulado == DialogResult.OK)
                    {
                        //string _DBFNETdll = "DBF.NET.dll"; ;
                        //string _GlobalSoluciondll = "GlobalSolucion.dll";
                        string _TransmisionNetBasicodll_X = "Transmision.Net.Basico.Oracle.dll";
                        ...
                        Byte[] _TransmisionNetBasico_X = null;
                        _TransmisionNetBasico_X = File.ReadAllBytes(_ruta_desarrollo + "\\" + _TransmisionNetBasicodll_X);

                        //File.WriteAllBytes(@_ruta_produccion + "\\" + _DBFNETdll, _archivo_DBFNET);
                        //File.WriteAllBytes(@_ruta_produccion + "\\" + _GlobalSoluciondll, _archivo_GlobalSolucion);
                        File.WriteAllBytes(@_ruta_produccion + "\\" + _TransmisionNetBasicodll_X, _TransmisionNetBasico_X);
                ...
                {
                    MessageBox.Show("No se puede actualizar a la misma version de la dll basico XSTORE", "Bata Peru Sistema...", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                }
